...
public class ArrayTabulatedFunction extends AbstractTabulatedFunction implements Insertable, Removable {
    ...
    public ArrayTabulatedFunction(double[] xValues, double[] yValues) {
        if (xValues.length < 1 || yValues.length < 1 || xValues.length != yValues.length) {
            throw new IllegalArgumentException();
        }
        this.xValues = Arrays.copyOf(xValues, xValues.length);
        this.yValues = Arrays.copyOf(yValues, yValues.length);
        this.count = this.xValues.length; // Ensure count is set correctly
    }
    ...
    public double extrapolateLeft(double x) {
        if (count == 1) {
            return getY(0); // Correct for single point
        } else if (count >= 2) {
            // Implement logic for two or more points if needed
    ...
    protected double interpolate(double x, int floorIndex) {
        ...
        double lX = getX(floorIndex);
        double lY = getY(floorIndex);
        double rX = getX(floorIndex + 1);
        double rY = getY(floorIndex + 1);
        return interpolate(x, lX, rX, lY, rY);
    }
    ...
    public Iterator<Point> iterator() {
        return new Iterator<Point>() {
            ...
            public Point next() {
                ...
                Point point = new Point(xValues[CurrIndex], yValues[CurrIndex]);  // Создаем Point из текущего узла
                ++CurrIndex; // Перемещаемся на следующий узел
